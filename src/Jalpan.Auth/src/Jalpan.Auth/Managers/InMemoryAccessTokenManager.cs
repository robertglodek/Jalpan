    ...
    IOptions<AuthOptions> options) : IAccessTokenManager
{
    ...
    public Task<bool> IsCurrentActiveToken(CancellationToken cancellationToken = default)
        => IsActiveAsync(GetCurrentAsync());

    public Task DeactivateCurrentAsync(CancellationToken cancellationToken = default)
        => DeactivateAsync(GetCurrentAsync());

    public Task<bool> IsActiveAsync(string token, CancellationToken cancellationToken = default)
        => Task.FromResult(string.IsNullOrWhiteSpace(_cache.Get<string>(GetCacheKey(token))));

    public Task DeactivateAsync(string token, CancellationToken cancellationToken = default)
    {
