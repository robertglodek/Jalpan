...
internal sealed class PayloadTransformer : IPayloadTransformer
{
    ...
    public PayloadTransformer(GatewayOptions options, IPayloadManager payloadManager, IValueProvider valueProvider)
    {
        _options = options;
    ...
    public bool HasTransformations(string resourceId, Configuration.Route route)
    {
    ...
    public PayloadSchema Transform(string payload, string resourceId, Configuration.Route route, HttpRequest request, RouteData data)
    {
    ...
    private string GetPayloadKey(Configuration.Route route) => _payloadManager.GetKey(route.Method, route.Upstream);
}